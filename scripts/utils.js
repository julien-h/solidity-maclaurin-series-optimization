...
async function runComparison(
  ...
  return runComparison_(
    (decimal, reciprocalOfInterestRate, a, b, precision) => runTestCase(baseline, decimal, reciprocalOfInterestRate, a, b, precision),
    (decimal, reciprocalOfInterestRate, a, b, precision) => runTestCase(optimized, decimal, reciprocalOfInterestRate, a, b, precision),
    iTestCase,
    decimal,
    reciprocalOfInterestRate,
    a,
 ...
 */
  async function runComparison_(
    ...
    let optimizedGas = 0;
    try {
      let data = await baselineCall(decimal, reciprocalOfInterestRate, a, b, precision);
    ...
    try {
      let data = await optimizedCall(decimal, reciprocalOfInterestRate, a, b, precision);
    ...
    if (baselineError && optimizedError) {
      ...
      if (same) {
        console.log(`📗 Testcase ${iTestCase} (prec=${precision}): both contracts triggered an error: ${baselineError}`);
      } else {
        console.log(`📗 Testcase ${iTestCase} (prec=${precision}): both contracts triggered an error:`);
        console.log(`- baseline:  ${baselineError}`);
        console.log(`- optimized: ${optimizedError}`);
        console.log("");
      }
    } else if ((baselineError && !optimizedError) || (!baselineError && optimizedError)) {
      const prefix = (baselineError && !optimizedError) ? "📙 Difference" : "📕 Error";
      ...
      console.log(`Parameters: ${decimal}, ${reciprocalOfInterestRate}, ${a}, ${b}, ${precision}`);
      console.log("");
      ...
      console.log(`📗 Testcase ${iTestCase} (prec=${precision}) -EXACT- savings:  ${gasSavedRelative} %  |  ${gasSaved} gas units`);
    } else if (baselineResult.sub(optimizedResult).abs() < 99) {
      const diff = baselineResult.sub(optimizedResult).abs();
      const gasSaved = optimizedGas.sub(baselineGas);
      const gasSavedRelative = gasSaved.mul(100).div(baselineGas);
      console.log(`📗 Testcase ${iTestCase} (prec=${precision}) EPSILON savings:  ${gasSavedRelative} %  |  ${gasSaved} gas units | delta = ${diff}`);
...
module.exports ={
    runComparison, runComparison_, runTestCase, runTestCaseV3
}